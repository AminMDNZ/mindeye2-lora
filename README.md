...
[![DOI]([![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.22831388.svg)](https://doi.org/10.5281/zenodo.22831388)
...
**Answer, measured:** a low-rank update is not merely enough, it is better. Across 6 arms
× 3 seeds, every LoRA rank outperformed full fine-tuning on two-way CLIP identification —
r=4 by +0.0127 [+0.0099, +0.0159] while training 2.56% of the parameters — and accuracy
was flat from r=4 to r=64. With 688 training trials, updating 729M parameters overfits.
Full numbers, caveats and the deviations from the published recipe: [docs/RESULTS.md](docs/RESULTS.md).
...
**[docs/GUIDE.md](docs/GUIDE.md)** is the full implementation guide: what each stage does,
how to change the experiment, how to add a new adaptation method, and how to run it off
Colab. **[docs/RESULTS.md](docs/RESULTS.md)** has the measured numbers.
...
the headroom that full fine-tuning creates over the frozen baseline, and a **retention
ratio**:
...
**Retention only means something when the reference arm is the ceiling.** If another arm
beats it, or the frozen→full headroom is small, the ratio divides by a near-zero
denominator and returns nonsense like 292%. `build_report` detects both cases and leads
with the raw ranking instead — which is what happened in this experiment.
...
docs/               GUIDE.md · EXPERIMENT_DESIGN.md · RESULTS.md · figures/
```
...
Generated by `cli report`; copies of the run that produced [docs/RESULTS.md](docs/RESULTS.md)
are in [`docs/figures/`](docs/figures).

| | |
|---|---|
| ![per-image metrics](docs/figures/metric_bars.png) | ![paired differences](docs/figures/paired_differences.png) |
| Per-image means with 95% CIs. Both metrics are higher-is-better; the y-axis is zoomed, so bar height is not proportional to score. | Paired differences against full fine-tuning. Every LoRA arm sits right of zero. |
| ![training loss](docs/figures/training_loss.png) | ![cost/quality](docs/figures/cost_quality.png) |
| Full fine-tuning reaches a far lower *training* loss while generalising worst — the overfitting signature. | Accuracy against trainable parameters. |

![reconstructions](docs/figures/reconstructions.png)

Reconstructions from the frozen SDXL unCLIP decoder: stimulus on top, one row per arm.
...
checkpoints and decoder are from **MindEye2** (Scotti et al., ICML 2024) and are imported
...
- Using **NSD** requires accepting its [terms and
  conditions](https://cvnlab.slite.page/p/IB6BSeW_7o/Terms-and-Conditions) first. They
  restrict redistribution and require citation.
- The **SDXL unCLIP decoder** is under CreativeML Open RAIL++-M, which carries use
  restrictions MIT does not.
- The **SwAV** model used by one optional image metric is CC BY-NC — non-commercial.

Full inventory: [THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md). This repository
redistributes no weights, no fMRI data and no upstream code; everything is fetched at
