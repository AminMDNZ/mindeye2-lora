...
def figure_forest(comparisons: list[dict], out: Path,
                  metric_filter: Sequence[str] | None = None) -> Path:
    """Paired difference vs the full fine-tune, with bootstrap CIs.

    Panels share a y-axis and only the leftmost carries arm labels: repeating them on
    every panel makes them collide with the neighbouring axes. The equivalence zone is
    explained once in the caption rather than by a legend in each panel, which
    previously sat on top of the data points.
    """
    plt = _mpl()
    rows = [c for c in comparisons if not metric_filter or c["metric"] in metric_filter]
    if not rows:
        return out
    metrics = [m for m in (metric_filter or sorted({r["metric"] for r in rows}))
               if any(r["metric"] == m for r in rows)]
    arms = _order(sorted({r["arm"] for r in rows}))
    y_of = {a: i for i, a in enumerate(reversed(arms))}

    height = max(2.4, 0.55 * len(arms) + 1.6)
    fig, axes = plt.subplots(1, len(metrics), figsize=(3.3 * len(metrics), height),
                             sharey=True)
    axes = np.atleast_1d(axes)
    ...
    for ax, metric in zip(axes, metrics):
        ...
        y = [y_of[r["arm"]] for r in sub]
        centers = [r["diff"] for r in sub]
        lo = [max(0.0, r["diff"] - r["ci_low"]) for r in sub]
        hi = [max(0.0, r["ci_high"] - r["diff"]) for r in sub]
        margin = next((r["tost_margin"] for r in sub
                       if np.isfinite(r.get("tost_margin", np.nan))), None)
        if margin:
            ax.axvspan(-margin, margin, color="#7a9e5e", alpha=0.12, zorder=0)
            shaded = True
        ax.axvline(0, color="#c1440e", lw=1, zorder=1)
        ax.errorbar(centers, y, xerr=[lo, hi], fmt="o", color="#2f6fb2", capsize=3,
                    zorder=3, markersize=5)
        ax.set_yticks(range(len(arms)))
        ax.set_yticklabels(list(reversed(arms)))
        ax.set_ylim(-0.6, len(arms) - 0.4)
        ax.set_xlabel(f"Δ {metric}")
        ax.margins(x=0.18)

    caption = "Paired differences vs full fine-tuning (negative = worse)"
    if shaded:
        caption += "\nshaded band = equivalence zone; red line = no difference"
    fig.suptitle(caption, y=1.0, fontsize=9)
    fig.tight_layout()
    fig.savefig(out)
    plt.close(fig)
    return out
...
def figure_qualitative(ground_truth, recons_by_arm: dict[str, "np.ndarray"], out: Path,
                       n: int = 8) -> Path:
    ...
    fig.suptitle("Reconstructions from the frozen SDXL unCLIP decoder", y=1.01, fontsize=9)
    fig.savefig(out)
    plt.close(fig)
    return out
