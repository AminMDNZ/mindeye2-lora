...
> **Outcome (measured).** The third case below is what occurred, in an unanticipated
> direction: every LoRA rank *exceeded* full fine-tuning, so the reference arm was not the
> ceiling and the retention framing does not apply. See [RESULTS.md](RESULTS.md).
